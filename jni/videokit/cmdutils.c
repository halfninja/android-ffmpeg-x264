...
{
__android_log_print(ANDROID_LOG_WARN, TAG,  "Good day sir and welcome to parse_options.\n");

    const char *opt, *arg;
    int optindex, handleoptions=1;
    const OptionDef *po;

__android_log_print(ANDROID_LOG_WARN, TAG,  "Pre prepare_app_arguments\n");

    /* perform system-dependent conversions for arguments list */
    prepare_app_arguments(&argc, &argv);

__android_log_print(ANDROID_LOG_WARN, TAG,  "Post prepare_app_arguments\n");
	...
	char buf[128] = "";
	sprintf(buf, "optindex %d - opt = %s\n", optindex-1, opt);
	__android_log_print(ANDROID_LOG_WARN, TAG,  buf);
	    ...
	    __android_log_print(ANDROID_LOG_WARN, TAG,  "got po");
...
unknown_opt:
	    ...
	    __android_log_print(ANDROID_LOG_WARN, TAG,  "OPT_EXIT.\n");
                return;
	    }
        } else {
            if (parse_arg_function) {
                if (parse_arg_function(NULL, opt) < 0) {
		    __android_log_print(ANDROID_LOG_WARN, TAG,  "parse_arg_function went bad.\n");
                    return;
		}
            }
        }
