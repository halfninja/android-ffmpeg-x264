...
static int opt_input_file(const char *opt, const char *filename)
{
   ...
   // ap->sample_fmt = audio_sample_fmt; //FIXME:not implemented in libavformat
    ...
    if (ret < 0 && verbose >= 0) {
        LOGE(stderr, "%s: could not find codec parameters\n", filename);
        av_close_input_file(ic);
        ffmpeg_exit(1);
    }
...
static void av_init(JNIEnv *env, jobject obj) {	
	...
	__android_log_print(ANDROID_LOG_INFO, TAG,  "Oh hey guys\n");
...
__android_log_print(ANDROID_LOG_INFO, TAG,  "Parsin yo options\n");
...
static void av_setInputFile(JNIEnv *env, jobject obj, jstring filePath) {
	const char *_filePath = (*env)->GetStringUTFChars(env, filePath, NULL);
	opt_input_file(" ", _filePath);
}

static void av_setOutputFile(JNIEnv *env, jobject obj, jstring filePath) {
	const char *_filePath = (*env)->GetStringUTFChars(env, filePath, NULL);
	opt_output_file(" ", _filePath);
}

//char bugsbuffer[128];
//#define bugs(fmt, ...) sprintf(bugsbuffer, fmt, ## __VA_ARGS__); __android_log_print(ANDROID_LOG_INFO, TAG,  bugsbuffer);
...
//    bugs("Hey guys what is up");

    if (transcode(output_files,
                     nb_output_files,
                     input_files,
                     nb_input_files,
                     stream_maps,
...
static JNINativeMethod methods[] = {
	...
	{ "native_av_setInputFile", "()V", (void*) av_setInputFile},
	{ "native_av_setOutputFile", "()V", (void*) av_setOutputFile}
};
...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    //__android_log_print(ANDROID_LOG_INFO, TAG, "Attaching files to logbox");   
	//FILE* x = freopen( "/sdcard/stderr.txt", "w", stderr );
	//FILE* y = freopen( "/sdcard/stdout.txt", "w", stdout );
	//__android_log_print(ANDROID_LOG_INFO, TAG, "Done the loggy log thing"); 
